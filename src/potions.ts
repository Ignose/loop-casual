...
import { castAugustScepterBuffs } from "./scepter";
...
const validPawWishes: Map<Effect, string> = new Map(
  wishableEffectData
    .filter(
      ({ e, name }) =>
        e !== $effect`Eau d' Clochard`) // hardcoded heuristics
...
export class Potion {
  ...
  value(
    ...
    const ascending = myDaycount() === 2;
    const totalTurns = myAdventures();
    const values: {
      name: PotionTier;
      quantity: number;
      value: number;
...
export function potionSetup(targetsOnly: boolean): void {
  ...
  withLocation($location.none, () => {
    const targets = targettingMeat() ? 75 : 0;
...
class VariableMeatPotion {
  ...
  getOptimalNumberToUse(yachtzees: number, targets: number): number {
    const barfTurns = Math.max(0, myAdventures() - yachtzees - targets);
